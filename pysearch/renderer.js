...
Renderer.prototype.getBitMask = function (query, text) {
  ...
  for (var i = 0; i < queryTokens.length; i++) {
    ...
    for (
      var cursor = text.toLowerCase().indexOf(queryToken.toLowerCase());
      cursor !== -1;
      cursor = text
        .toLowerCase()
        .indexOf(queryToken.toLowerCase(), cursor + queryToken.length)
    ) {
...
Renderer.prototype.addTags = function (query, text) {
  ...
  for (var i = 0; i < xmlSpecial.length; i++) {
    ...
    for (
      var cursor = text.indexOf(target, cursor);
      cursor !== -1;
      ...
      text =
        ...
        xmlEntities[target] +
        text.substring(cursor + target.length);
