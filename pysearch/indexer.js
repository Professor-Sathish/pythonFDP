...
Indexer.prototype.searchWithin = function (queryTokens, items) {
  return items.filter(function (item) {
    for (var i = 0; i < queryTokens.length; i++) {
      if (
        item.toLowerCase().indexOf(queryTokens[i].toLowerCase()) === -1 &&
        this.indexData[item].description
          .toLowerCase()
          .indexOf(queryTokens[i].toLowerCase())
      ) {
        return false;
      }
    }
...
Indexer.prototype.rank = function (queryTokens, items) {
  ...
  var numItems =
    rankedItems.length < this.maxItems ? rankedItems.length : this.maxItems;
...
Indexer.prototype.search = function (query) {
  ...
  for (var i = startIndex; i < query.length; i++) {
    ...
    this.resultsCache[i + 1] = this.searchWithin(
      queryTokens,
      this.resultsCache[i]
    );
  }
