  ...
  function setDefaultSuggestion(text) {
    if (text) {
      chrome.omnibox.setDefaultSuggestion({
        description: "<url><match>Java API Search</match></url> " + text,
      });
    } else {
      chrome.omnibox.setDefaultSuggestion({
        description: "<url><match>Java API Search</match></url>",
      });
    }
  }
    ...
    if (stripped_text.length >= 2) {
      suggestions.push({
        ...
        description: [
          'Search for "<match>',
          stripped_text,
          '</match> <dim>lang:java</dim>" using <match><url>Google Code Search</url></match> - <url>http://code.google.com/codesearch#search/&amp;q=',
          encodeURIComponent(stripped_text + " lang:java"),
          "</url>",
        ].join(""),
      });
  ...
  chrome.omnibox.onInputEntered.addListener(function (text) {
    ...
    if (stripped_text.endsWith(google_codesearch_suffix)) {
      ...
      navigateTo(
        "http://code.google.com/codesearch#search/&q=" +
          encodeURIComponent(newquery + " lang:java")
      );
      return;
    }
    ...
    navigateTo(
      "http://www.google.com/search?q=" +
        encodeURIComponent("Java " + stripped_text)
    );
  });
})();
