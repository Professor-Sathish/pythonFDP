(function () {
    ...
    req.onreadystatechange = function () {
      if (req.readyState == 4) {
        ...
        if (status == 200 || status == 301 || status == 302) {
          ...
          ifnotexists(url, req);
          setTimeout(function () {
            xhr(url, ifexists, ifnotexists, retry_time + 5).send(null);
          }, retry_time);
        }
      }
    };
  ...
  function setDefaultSuggestion(text) {
    if (text) {
      chrome.omnibox.setDefaultSuggestion({
        description: "<url><match>Python3 Search</match></url> " + text,
      });
    } else {
      chrome.omnibox.setDefaultSuggestion({
        description: "<url><match>Python3 Search</match></url>",
      });
    }
  }
    ...
    if (localStorage.hasOwnProperty("python_builtin_functions")) {
      ...
      xhr(
        ...
        function (url, req) {
          ...
          builtin_functions_ = {};
          builtin_functions_["builtin functions"] = {
          ...
          var matches = text.match(
            new RegExp(
              '<a class="[^"]*" href="[^"]*" title="[^"]*"><code class="xref py py-func docutils literal notranslate"><span class="pre">[^<]*</span></code></a>',
              "g"
            )
          );
          ...
          if (matches) {
            for (var i = 0; i < matches.length; ++i) {
              ...
              var url =
                hashidx == 0
                  ? "http://docs.python.org/3/library/functions.html" + href
                  : "http://docs.python.org/3/library/" + href;
      ...
      xhr(
        ...
        function (url, req) {
          ...
          constants_["constants"] = {
            name: "Constants",
            url: "http://docs.python.org/3/library/constants.html",
          };
          var text = req.responseText;
          var matches = text.match(
            new RegExp(
              '<a class="headerlink" href="#[^"]*" title="Permalink to this definition">[^"]*</a>',
              "g"
            )
          );
          for (var i = 0; i < matches.length; ++i) {
    ...
    if (localStorage.hasOwnProperty("python_types")) {
      ...
      xhr(
        ...
        function (url, req) {
          ...
          types_["types"] = {
            name: "Types",
            url: "http://docs.python.org/3/library/stdtypes.html",
          };
          var text = req.responseText;
          var matches = text.match(
            new RegExp(
        ...
        function (url, req) {
          ...
          for (var i = 0; i < matches.length; ++i) {
            ...
            if (name.startsWith("object.")) {
              ...
              datamodel_[shortname.toLowerCase()] = {
                name: shortname,
                url: url + href,
              };
    ...
    if (localStorage.hasOwnProperty("python_modules")) {
      ...
      xhr(
        ...
        function (url, req) {
          ...
          var matches = text.match(
            new RegExp(
              '<a href="library/[^"]*.html#module-[^"]*"><code class="xref">[^<]*</code></a></td><td>\n?.*<em>[^<]*</em>',
              "g"
            )
          );
      ...
      function (url, req) {
        ...
        if (matches) {
          for (var i = 0; matches !== null && i < matches.length; ++i) {
            ...
            var href = match.substring(hrefstartidx, hrefendidx);
            var name = href.substr(1);
            moduleobj["moduledata"][name.toLowerCase()] = {
              name: name,
              url: url + href,
            };
          }
  ...
  chrome.omnibox.onInputChanged.addListener(function (text, suggest_callback) {
    ...
    if ("print".startsWith(qlower)) {
      suggestions.push({
        ...
        description:
          "<match>print<match> - <url>http://docs.python.org/release/3.0.1/library/functions.html#print</url>",
      });
    }
    ...
    if (stripped_text.length >= 2) {
      suggestions.push({
        ...
        description: [
          'Search for "<match>',
          stripped_text,
          '</match> <dim>lang:python</dim>" using <match><url>Google Code Search</url></match> - <url>http://code.google.com/codesearch#search/&amp;q=',
          encodeURIComponent(stripped_text + " lang:python"),
          "</url>",
        ].join(""),
      });
      suggestions.push({
        ...
        description: [
          'Search for "<match>',
          stripped_text,
          '</match>" using <match><url>Develoment and Coding Search</url></match> - <url>http://www.google.com/cse?cx=005154715738920500810:fmizctlroiw&amp;q=',
          encodeURIComponent(stripped_text),
          "</url>",
        ].join(""),
      });
    }
  ...
  chrome.omnibox.onInputEntered.addListener(function (text) {
    ...
    if (
      ...
      stripped_text.startsWith("https://")
    ) {
      nav(stripped_text);
      return;
    }
    ...
    if (
      stripped_text.startsWith("www.") ||
      stripped_text.endsWith(".com") ||
      stripped_text.endsWith(".net") ||
      stripped_text.endsWith(".org") ||
      stripped_text.endsWith(".edu")
    ) {
    ...
    if (stripped_text.endsWith(google_codesearch_suffix)) {
      var newquery = stripped_text
        .substring(0, stripped_text.length - google_codesearch_suffix.length)
        .strip();
      nav(
        "http://code.google.com/codesearch#search/&q=" +
          encodeURIComponent(newquery + " lang:python")
      );
      return;
    }
    ...
    if (stripped_text.endsWith(devsearch_suffix)) {
      var newquery = stripped_text
        .substring(0, stripped_text.length - devsearch_suffix.length)
        .strip();
      nav(
        "http://www.google.com/cse?cx=005154715738920500810:fmizctlroiw&q=" +
    ...
    nav(
      "http://www.google.com/search?q=" +
        encodeURIComponent("Python " + stripped_text)
    );
  });
})();
